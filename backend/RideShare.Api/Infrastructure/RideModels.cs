...
public sealed record DriverAvailableRide(Guid RideId, Guid CustomerId, string CustomerName, string CustomerPhone, double PickupLat, double PickupLng, double DestinationLat, double DestinationLng, decimal EstimatedFare, RideStatus Status, DateTimeOffset RequestedAt);
public sealed record DriverContactInfo(Guid DriverId, string Name, string Phone, string VehicleType, double Rating);
